...
class AddTenantModal extends Component
{
    ...
    private function findBlockedMatch(): array
    {
        ...
        $matchedBy = match (true) {
            $email !== '' && strcasecmp($blocked->email, $email) === 0
                => 'email',
            $phone && $blocked->contact === $phone
                => 'phone number',
            $govIdType !== '' && $govIdNum !== ''
                && strcasecmp((string) $blocked->government_id_type, $govIdType) === 0
                && (string) $blocked->government_id_number === $govIdNum
                => 'government ID',
            $fullName !== '' && $birthdate
                && strtolower(trim($blocked->first_name . ' ' . $blocked->last_name)) === $fullName
                && optional($blocked->birthdate)->format('Y-m-d') === $birthdate
                => 'full name + birthdate',
