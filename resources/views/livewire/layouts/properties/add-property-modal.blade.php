<div>
    @if ($isOpen)
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
            <div
                class="relative w-full max-w-2xl bg-gray-50 rounded-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
                ...
                <div class="bg-[#070589] text-white p-6 flex-shrink-0">
                    <div class="flex items-start justify-between">
                        ...
                        <flux:tooltip :content="'Close property form without saving'" position="bottom">
                            <button type="button" x-on:click="$dispatch('open-modal', 'discard-property-confirmation')"
                ...
                {{-- Scrollable Content --}}
                <div class="flex-1 overflow-y-auto p-6" x-data
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                            <div>
                                ...
                                <div class="relative mb-6">
                                    <input wire:model.defer="buildingName" type="text" id="buildingName"
                                ...
                                <div class="mb-6">
                                    <x-forms.address-picker variant="floating" label="Address" :required="true"
                                        province-model="provinceId" city-model="cityId" barangay-model="barangayId"
                                        street-model="street" :province-id="$provinceId" :city-id="$cityId" :barangay-id="$barangayId"
                                        :street="$street" :provinces="$this->psgcProvinces()" :cities="$this->psgcCities($provinceId)" :barangays="$this->psgcBarangays($cityId)" />
                                </div>
                            ...
                            <div class="mt-4">
                                <div class="relative">
                                    <input type="number" step="0.01" min="0" max="10"
                                        wire:model.defer="depositInterestRate" id="depositInterestRate"
                                        class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-[#0030C5] peer"
                                        placeholder=" " />
                                    <label for="depositInterestRate"
                                        class="absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-[#0030C5] peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-1">
                                        Security Deposit Interest Rate (% per year)
                                    </label>
                                    <p class="text-[10px] text-gray-400 mt-1">Based on your depository bank's prevailing
                                        savings rate (RA 9653 IRR &sect;7b). Leave blank or 0 if not applicable.</p>
                                </div>
                            </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 mb-6">
                                    Upload up to 5 photos of your property. <span class="text-red-500">*</span> (max
                                    10MB each)
                                </p>
                                ...
                                <div x-data="{
                                    ...
                                    <template x-if="totalCount > 0">
                                        <div class="grid grid-cols-4 gap-3 mb-3">
                                            ...
                                            @foreach ($existingPhotos as $photo)
                                                <div
                                                    class="relative group rounded-lg overflow-hidden border border-gray-300 aspect-square bg-gray-50">
                                                    <img src="{{ $photo['url'] }}" class="w-full h-full object-cover"
                                                        alt="{{ $photo['name'] }}">
                                                    <flux:tooltip :content="'Remove this photo from the upload'"
                                                        position="bottom">
                                                        <button type="button"
                                            ...
                                            <template x-for="(src, i) in previews" :key="i">
                                                <div
                                                    ...
                                                    <flux:tooltip :content="'Remove this photo from the upload'"
                                                        position="bottom">
                                                        <button type="button" @click.prevent="removePreview(i)"
                                                            class="absolute top-1.5 right-1.5 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                                                viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <template x-if="totalCount < 5">
                                                <label
                                                    class="relative cursor-pointer flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 aspect-square bg-gray-50 hover:bg-gray-100 transition-colors">
                                                    ...
                                                    <input type="file" wire:model="newPhotos" accept="image/*"
                                    ...
                                    <template x-if="totalCount === 0">
                                        <div
                                            class="relative border border-dashed border-gray-300 rounded-lg p-8 bg-gray-50 text-center hover:bg-gray-100 transition-colors cursor-pointer">
                                            ...
                                            <div class="flex flex-col items-center pointer-events-none">
                                                ...
                                                <p class="text-sm text-gray-500">Drag & drop or click to upload photos
                                                </p>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                            ...
                            <p class="text-xs text-gray-500 mb-5">Required <span class="text-red-500">*</span> — PDF
                                or image, max 10MB each</p>
                            ...
                            @php
                                $allDocs = [
                                    ...
                                    [
                                        ...
                                        'label' => 'Transfer Certificate / Deed of Sale',
                                    ...
                                    [
                                        'field' => 'occupancyPermit',
                                        'label' => 'Occupancy Permit',
                                        'category' => 'occupancy_permit',
                                        'group' => 'tenant',
                                    ],
                                ];
                            @endphp
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                @foreach ($allDocs as $doc)
                                    @php
                                        $docField = $doc['field'];
                                        $docLabel = $doc['label'];
                                        $existingDoc = collect($existingDocuments)->firstWhere(
                                            'category',
                                            $doc['category'],
                                        );
                                    ...
                                    <div x-data="{
                                        ...
                                        <p class="text-sm font-medium text-gray-700 mb-2">{{ $docLabel }} <span
                                        ...
                                        @if ($existingDoc)
                                            ...
                                            <div class="flex items-center justify-between gap-2">
                                                <div class="flex items-center gap-2 min-w-0">
                                                    ...
                                                    <span
                                                        class="text-xs text-gray-600 truncate">{{ $existingDoc['name'] }}</span>
                                                </div>
                                                <div class="flex items-center gap-1.5 flex-shrink-0">
                                                    <label
                                                        class="cursor-pointer text-xs text-[#2360E8] hover:text-[#1d4eb8] font-medium transition-colors">
                                                        Replace
                                                        <input type="file" wire:model="{{ $docField }}"
                                                            accept=".pdf,.jpg,.jpeg,.png"
                                                            @change="validateFile($event)" class="hidden">
                                                    </label>
                                                    <flux:tooltip :content="'Remove this document from the upload'"
                                                        position="bottom">
                                                        <button type="button"
                                                            wire:click="removeExistingDocument({{ $existingDoc['id'] }})"
                                        ...
                                        @elseif($this->{$docField})
                                            ...
                                            <div class="flex items-center justify-between gap-2">
                                                <div class="flex items-center gap-2 min-w-0">
                                                    ...
                                                    <span
                                                        class="text-xs text-gray-600 truncate">{{ $this->{$docField}->getClientOriginalName() }}</span>
                                                </div>
                                                <flux:tooltip :content="'Remove this document from the upload'"
                                                    position="bottom">
                                                    <button type="button"
                                                        wire:click="$set('{{ $docField }}', null)"
                                        ...
                                        @else
                                            ...
                                            <label class="flex items-center gap-2 cursor-pointer group">
                                                <div
                                                    class="w-8 h-8 rounded-lg bg-white border border-dashed border-gray-300 flex items-center justify-center group-hover:border-[#2360E8] group-hover:bg-blue-50 transition-colors flex-shrink-0">
                                                    <svg class="w-4 h-4 text-gray-400 group-hover:text-[#2360E8] transition-colors"
                                                ...
                                                <span
                                                    class="text-xs text-gray-400 group-hover:text-gray-600 transition-colors">Choose
                                                    file...</span>
                                                <input type="file" wire:model="{{ $docField }}"
                                                    accept=".pdf,.jpg,.jpeg,.png" @change="validateFile($event)"
                                                    class="hidden">
                                            </label>
                                        @endif
                                        ...
                                        <div x-show="uploading" x-cloak class="mt-2">
                                            ...
                                            <p class="text-[11px] text-gray-400 mt-1"
                                                x-text="progress < 100 ? 'Uploading... ' + progress + '%' : 'Complete'">
                                            </p>
                                        </div>
                ...
                <div class="flex-shrink-0 bg-gray-50 border-t border-gray-200 px-6 py-4">
                    <div class="flex justify-end">
                        <button type="button" wire:click="validateAndConfirm"
        ...
        {{-- 1. SAVE CONFIRMATION  --}}
        <x-ui.modal-confirm name="save-property-confirmation" title="Save Property?"
            description="Are you sure you want to add this new property?" confirmText="Yes, Save" cancelText="Cancel"
            confirmAction="next" />
        ...
        <div x-data="{ show: false }" x-show="show"
            ...
            class="fixed inset-0 z-[60] flex items-center justify-center px-4 py-6 sm:px-0" style="display: none;">
            <div x-show="show" class="fixed inset-0 transform transition-all" x-on:click="show = false">
                <div class="absolute inset-0 bg-gray-600 opacity-50"></div>
            </div>

            <div x-show="show"
                class="bg-white rounded-[20px] overflow-hidden shadow-xl transform transition-all sm:w-full sm:max-w-[480px] p-8 relative z-[100]">
                ...
                <div class="text-center mt-4 mb-8">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed px-4">Are you sure you want to close? All details
                        will be lost.</p>
                </div>
