<div class="bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden"
    style="font-family: 'Open Sans', sans-serif;" x-data="{ lightbox: false, lightboxSrc: '', confirmDelete: null }">
    ...
    @if ($ticket)
        @php
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::warning(
                    'Manager maintenance detail lease info query failed in Blade.',
                    [
                        'request_id' => $ticket->request_id ?? null,
                        'lease_id' => $ticket->lease_id ?? null,
            ...
            $tenantName = $leaseInfo ? $leaseInfo->first_name . ' ' . $leaseInfo->last_name : $ticket->logged_by ?? '—';
        ...
        <div class="flex flex-col h-full">
            ...
            {{-- 1. Fixed Header --}}
            <div class="flex-shrink-0 rounded-t-3xl z-10 overflow-hidden"
                style="background: linear-gradient(135deg, #070589 0%, #0a1ea8 40%, #2360E8 100%);">
                <div class="relative p-6">
                        ...
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center"
                            ...
                            <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                ...
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M11.42 15.17l-5.78 3.04 1.1-6.44-4.68-4.56 6.47-.94L11.42 0l2.89 5.87 6.47.94-4.68 4.56 1.1 6.44z" />
                            </svg>
                        </div>
                        <span class="text-xs font-semibold uppercase tracking-widest"
                            style="color: rgba(191,219,254,0.9);">Maintenance Request</span>
                    </div>

                    {{-- Tenant Name + Unit --}}
                    <div class="flex items-center gap-3 mb-3">
                        <h3 class="text-white font-bold text-2xl leading-tight">{{ $tenantName }}</h3>
                        <div class="rounded-lg px-3 py-1 flex items-center gap-1.5"
                            style="background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(8px);">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <div class="flex items-center gap-1.5 rounded-lg px-3 py-1.5"
                            ...
                            <svg class="w-3.5 h-3.5 flex-shrink-0" style="color: #93c5fd;" fill="currentColor"
                                viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z"
                                    clip-rule="evenodd" />
                            </svg>
                            <span class="text-xs truncate max-w-[220px]"
                                style="color: rgba(255,255,255,0.85);">{{ $addressDisplay }}</span>
                        </div>
                        <div class="flex items-center gap-1.5 rounded-lg px-3 py-1.5"
                            style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.06);">
                            <svg class="w-3.5 h-3.5 flex-shrink-0" style="color: #93c5fd;" fill="currentColor"
                        ...
                        <div class="flex items-center gap-1.5 rounded-lg px-3 py-1.5"
                            style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.06);">
                            <svg class="w-3.5 h-3.5 flex-shrink-0" style="color: #93c5fd;" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <span class="text-xs"
                                style="color: rgba(255,255,255,0.85);">{{ \Carbon\Carbon::parse($ticket->created_at)->format('M d, Y \a\t h:i A') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-5"
                ...
                <div
                    ...
                    <div class="flex items-center gap-2.5 mb-4">
                        <div class="w-8 h-8 rounded-xl bg-[#EEF2FF] flex items-center justify-center">
                            <svg class="w-4 h-4 text-[#2360E8]" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50">
                            <p class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1">Ticket
                                Number</p>
                            <p class="text-sm font-bold text-gray-800 font-mono">{{ $ticketIdDisplay }}</p>
                        </div>
                        <div class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50">
                            <p class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1">
                                Category</p>
                            <p class="text-sm font-bold text-gray-800">{{ $ticket->category ?? 'General Maintenance' }}
                            </p>
                        </div>
                        <div class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50">
                            <p class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1">Status
                            </p>
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $statusStyles }}">
                                {{ $ticket->status }}
                            </span>
                        </div>
                        <div class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50">
                ...
                <div
                    ...
                    <div class="flex items-center gap-2.5 mb-4">
                        <div class="w-8 h-8 rounded-xl bg-[#EEF2FF] flex items-center justify-center">
                            <svg class="w-4 h-4 text-[#2360E8]" fill="none" viewBox="0 0 24 24"
                    ...
                    <div
                        class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50 text-sm text-gray-700 leading-relaxed">
                        {{ $ticket->problem }}
                    </div>
                </div>
                ...
                <div
                    ...
                    <div x-show="lightbox" x-transition:enter="transition ease-out duration-200"
                        ...
                        style="display:none;">
                        <img :src="lightboxSrc" class="max-w-full max-h-full object-contain rounded-xl shadow-2xl"
                            @click.stop>
                        <flux:tooltip :content="'Close the image viewer'" position="bottom">
                            <button @click="lightbox = false"
                                class="absolute top-5 right-5 w-10 h-10 bg-white/20 hover:bg-white/30 text-white rounded-full flex items-center justify-center">
                    ...
                    @if (!empty($imagePaths))
                        <div class="grid {{ count($imagePaths) === 1 ? 'grid-cols-1' : 'grid-cols-2' }} gap-3">
                            @foreach ($imagePaths as $imgPath)
                                <div class="rounded-xl overflow-hidden border border-gray-100 shadow-sm cursor-zoom-in group relative"
                                    @click="lightbox = true; lightboxSrc = '{{ asset('storage/' . $imgPath) }}'">
                                    <img src="{{ asset('storage/' . $imgPath) }}" alt="Maintenance issue photo"
                                    ...
                                    <div
                                        ...
                                        <div
                                            class="opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 rounded-full p-1.5">
                    ...
                    @else
                        <div
                            class="rounded-xl border-2 border-dashed border-gray-200 bg-[#F8FAFF] flex flex-col items-center justify-center py-10 text-gray-400">
                ...
                @if ($isEditable || $hasTrackingData)
                    <div id="manage-request-section"
                        ...
                        @error('manageRequest')
                            <div
                                class="mb-4 px-4 py-3 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm font-medium flex items-center gap-2">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2.5">
                                ...
                                <h5 class="font-bold text-sm text-[#070589] uppercase tracking-wide">Manage Request
                                </h5>
                            </div>
                            @if ($isEditable && $allFieldsFilled)
                                <flux:tooltip :content="'Modify the request description and details'"
                                    position="bottom">
                                    <button @click="editing = !editing"
                                        class="flex items-center gap-1.5 bg-white text-[#2360E8] rounded-lg px-3 py-1.5 text-xs font-semibold hover:bg-[#EEF2FF] transition-colors border border-blue-100">
                                        <svg x-show="!editing" class="w-3.5 h-3.5" fill="none"
                        ...
                        <div x-show="!editing" class="grid grid-cols-3 gap-3">
                            <div class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50">
                                ...
                                <p class="text-sm font-bold text-gray-800">
                                    {{ $ticket->assigned_to ?? 'Not assigned' }}</p>
                            </div>
                            <div class="bg-[#F8FAFF] rounded-xl p-3.5 border border-blue-50">
                                <p class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1">
                                    Expected Completion</p>
                                <p class="text-sm font-bold text-gray-800">
                        ...
                        @if ($isEditable)
                            <div x-show="editing" x-cloak class="space-y-3">
                                <div>
                                    <label
                                        class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Change
                                        Priority</label>
                                    <select wire:model="newUrgency"
                                ...
                                <div>
                                    <label
                                        class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Assigned
                                        Worker / Vendor</label>
                                    <input type="text" wire:model="assignedTo"
                                        placeholder="e.g. Juan (Plumber), ABC Services" maxlength="255"
                                ...
                                <div>
                                    <label
                                        class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Expected
                                ...
                                <div class="flex justify-end">
                                    <button wire:click="saveManageRequest"
                                        class="px-5 py-2 text-xs font-bold text-white bg-[#070589] hover:bg-[#000060] rounded-xl transition-colors">
                                        Save Changes
                                    </button>
                                </div>
                            </div>
                ...
                <div id="cost-section"
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        @if (in_array($ticket->status, ['Ongoing', 'Completed', 'Resolved']))
                            <div class="flex items-center gap-2">
                                ...
                                <flux:tooltip :content="'Log a new maintenance expense'" position="bottom">
                                    <button wire:click="toggleCostForm"
                        ...
                        @elseif($ticket->status === 'Pending')
                            <span class="text-[11px] text-amber-500 font-medium">Mark as Ongoing to start tracking
                                costs</span>
                        @endif
                    </div>
                    ...
                    {{-- Cost Summary Card --}}
                    <div class="mb-4">
                        <div class="relative overflow-hidden rounded-2xl p-4 text-white shadow-sm"
                            style="background: linear-gradient(135deg, #070589, #0a1ea8);">
                            <div class="absolute top-0 right-0 w-16 h-16 rounded-full"
                                style="background: rgba(255,255,255,0.1); transform: translate(16px, -16px);"></div>
                            <div class="absolute bottom-0 left-0 w-10 h-10 rounded-full"
                                style="background: rgba(255,255,255,0.05); transform: translate(-12px, 12px);"></div>
                            <p class="text-[11px] uppercase font-bold tracking-wider mb-1" style="color: #bfdbfe;">
                                Total Cost</p>
                            <p class="text-2xl font-extrabold tracking-tight">
                                ...
                                @endphp
                                <span class="text-base font-bold mr-0.5"
                                    style="color: #bfdbfe;">PHP</span>{{ $parts[0] }}<span class="text-base"
                                    style="color: #bfdbfe;">.{{ $parts[1] }}</span>
                            </p>
                    ...
                    {{-- Add Cost Form --}}
                    @if ($showCostForm)
                        <div class="rounded-2xl p-5 mb-4 space-y-4"
                            style="background: linear-gradient(to bottom, rgba(238,242,255,0.8), white); border: 1px solid #c7d2fe;"
                            x-data x-init="$nextTick(() => $refs.costInput?.focus())">
                            <h4 class="text-sm font-bold text-[#070589] mb-1">
                                {{ $editingCostId ? 'Edit Cost Entry' : 'New Cost Entry' }}</h4>
                            <div>
                                <label
                                    class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Description</label>
                                <input type="text" wire:model="costDescription"
                                    placeholder="e.g. Labor - Plumber (2 hrs), Pipe fittings..." maxlength="255"
                            ...
                            <div>
                                <label
                                    class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Amount
                                    (PHP)</label>
                                <style>
                            ...
                            <div>
                                <label
                                    class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Charge
                                    To</label>
                                ...
                                <p
                                    class="text-[11px] mt-1.5 {{ $chargedTo === 'tenant' ? 'text-amber-600' : 'text-blue-500' }}">
                                    @if ($chargedTo === 'tenant')
                            ...
                            <div>
                                <label
                                    class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-1.5 block">Quick
                                    Select</label>
                            ...
                            <div class="flex justify-end gap-2">
                                ...
                                <button wire:click="saveCost" wire:loading.attr="disabled" wire:target="saveCost"
                                    ...
                                    <span wire:loading.remove
                                        wire:target="saveCost">{{ $editingCostId ? 'Update Cost' : 'Save Cost Entry' }}</span>
                    ...
                    @if (!empty($costItems))
                        <div class="space-y-2">
                            @foreach ($costItems as $index => $item)
                                <div
                                    class="group bg-[#F8FAFF] border border-blue-50 rounded-xl p-3.5 flex items-center gap-3 hover:shadow-sm transition-all duration-200">
                                    <div
                                        class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-[#EEF2FF]">
                                        <svg class="w-4 h-4 text-[#2360E8]" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                                        </svg>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-2 mt-0.5">
                                            ...
                                            <span
                                                class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wide
                                    ...
                                    <div
                                        ...
                                        <flux:tooltip :content="'Edit this expense entry'" position="bottom">
                                            <button wire:click="editCostItem({{ $item['log_id'] }})"
                                                ...
                                                <svg class="w-3.5 h-3.5 text-[#2360E8]" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                                        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            ...
                            @if (count($costItems) > 1)
                                <div class="rounded-xl p-3.5 flex items-center justify-between mt-1"
                                    style="background-color: #ecfdf5; border: 1px solid #d1fae5;">
                                    <span class="text-xs font-bold uppercase tracking-wide"
                                        style="color: #047857;">Request Total</span>
                                    <span class="text-base font-extrabold font-mono" style="color: #047857;">PHP
                                        {{ number_format($requestTotal, 2) }}</span>
                                </div>
                            @endif
                        </div>
                    @endif
                ...
                <template x-if="confirmDelete">
                    <div class="fixed inset-0 z-[9998] flex items-center justify-center bg-black/40"
                        ...
                        <div class="bg-white rounded-2xl shadow-2xl p-6 w-80 mx-4" @click.stop>
                            ...
                            <div class="flex gap-2">
                                <button @click="confirmDelete = null"
                                    class="flex-1 px-4 py-2 rounded-xl text-sm font-semibold bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">
                                    Cancel
                                </button>
                ...
                @if ($requestTotal >= \App\Livewire\Layouts\Maintenance\ManagerMaintenanceDetail::COST_WARNING_THRESHOLD)
                    <div class="flex items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-amber-600">This request's total (PHP
                                {{ number_format($requestTotal, 2) }}) exceeds the PHP
                                {{ number_format(\App\Livewire\Layouts\Maintenance\ManagerMaintenanceDetail::COST_WARNING_THRESHOLD, 0) }}
                                threshold. Please review before adding more costs.</p>
                        </div>
                    </div>
                @endif
                ...
                <div
                    ...
                    <div class="mb-3">
                        <input type="text" wire:model="noteText" wire:keydown.enter="saveNote"
                            placeholder="Add a note..." maxlength="1000"
                    ...
                    @if (!empty($notes))
                        <div class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                            @foreach ($notes as $n)
                                <div
                                    class="group flex items-start gap-3 bg-[#F8FAFF] border border-blue-50 rounded-xl p-3">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm text-gray-700">{{ $n['note'] }}</p>
                                        <p class="text-[11px] text-gray-400 mt-1">
                                            {{ $n['author_name'] }} &middot;
                                            {{ \Carbon\Carbon::parse($n['created_at'])->format('M d, h:i A') }}
                                        </p>
                                    </div>
                            ...
                            @foreach ($activities as $act)
                                ...
                                <div class="border-l-[3px] rounded-r-lg px-3 py-2.5 {{ $borderColor }}">
                                    ...
                                    <p class="text-[11px] text-gray-400 mt-0.5">
                                        {{ $act['actor_name'] }} &middot;
                                        {{ \Carbon\Carbon::parse($act['created_at'])->format('M d, h:i A') }}
                                    </p>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if ($ticket->status === 'Completed')
                                <span
                                    class="flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold bg-green-100 text-green-700">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="pl-2">
                        @if (in_array($ticket->status, ['Completed', 'Resolved']))
                            <div class="flex gap-4 relative pb-6">
                                ...
                                <div
                                    class="flex-shrink-0 w-5 h-5 rounded-full bg-green-500 border-2 border-white shadow z-10 mt-0.5">
                                </div>
                                <div class="flex-1">
                                    ...
                                    <div
                                        class="bg-[#F8FAFF] border border-blue-50 shadow-sm p-3 rounded-xl text-sm text-gray-600">
                        ...
                        @if (in_array($ticket->status, ['Ongoing', 'In Progress', 'Completed', 'Resolved']))
                            <div class="flex gap-4 relative pb-6">
                                <div class="absolute left-[9px] top-6 bottom-0 w-[2px] bg-gray-200"></div>
                                <div
                                    class="flex-shrink-0 w-5 h-5 rounded-full bg-[#070589] border-2 border-white shadow z-10 mt-0.5">
                                </div>
                                <div class="flex-1">
                        ...
                        <div class="flex gap-4 relative">
                            <div
                                class="flex-shrink-0 w-5 h-5 rounded-full bg-gray-300 border-2 border-white shadow z-10 mt-0.5">
                            </div>
                            <div class="flex-1">
                ...
                @if ($feedback)
                    ...
                    <div
                        ...
                        <div class="bg-[#F8FAFF] rounded-xl p-4 border border-blue-50 space-y-4">
                            <div>
                                <p class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-2">
                                    Overall Service Rating</p>
                                <div class="flex items-center gap-1.5">
                                    @for ($s = 1; $s <= 5; $s++)
                            ...
                            @if (!empty($tagList))
                                <div>
                                    <p
                                        class="text-[11px] font-semibold uppercase tracking-wider text-[#2360E8]/60 mb-2">
                                        Experience</p>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach ($tagList as $tag)
                                            <span
                                                class="px-3 py-1 rounded-full text-xs font-semibold bg-[#070589] text-white">
                                                {{ $tag }}
                                            </span>
                                        @endforeach
                                    </div>
                            ...
                            @if (!empty($feedback->comment))
                                <div>
                                    <p
                                        ...
                                        Comment</p>
                                    <div
                                        class="bg-white border border-blue-100 rounded-xl p-3 text-sm text-gray-700 leading-relaxed">
                                        {{ $feedback->comment }}
                                    </div>
                                </div>
                            @endif
                            ...
                            <p class="text-[11px] text-gray-400 pt-1 border-t border-blue-100">
                                Submitted
                                {{ \Carbon\Carbon::parse($feedback->created_at)->format('M d, Y \a\t h:i A') }}
                            </p>
                        </div>
                    </div>
                @elseif(in_array($ticket->status, ['Completed', 'Resolved']))
                    <div
                        ...
                        <div
                            class="rounded-xl border-2 border-dashed border-gray-200 bg-[#F8FAFF] py-8 flex flex-col items-center text-gray-400">
        ...
        <x-ui.modal-confirm name="confirm-mark-ongoing" title="Mark as Ongoing?"
        ...
        <x-ui.modal-confirm name="confirm-mark-completed" title="Mark as Completed?"
        ...
        <x-ui.modal-confirm name="confirm-revert-status" title="Revert Status?"
    ...
    @else
        ...
        <div class="flex-1 flex flex-col items-center justify-center text-gray-400 p-6">
            <div class="bg-[#EEF2FF] p-8 rounded-full mb-5">
                <svg class="h-16 w-16 text-[#2360E8] opacity-40" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
